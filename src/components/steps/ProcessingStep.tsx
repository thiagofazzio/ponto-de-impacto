...
// CORRIGIDO: não recebe mais onFinished. É puramente visual — cicla as
// mensagens e para/segura na última, "esperando". Quem decide quando a
// etapa realmente termina é o WizardContainer, quando a resposta da API
// chega de verdade (ver runDiagnosticCalculation).
  ...
  return (
    <div className="max-w-xl mx-auto my-12 p-8 bg-white border border-[#D8D3CB] rounded-3xl shadow-xl text-center space-y-8">
      ...
      <div className="w-full bg-[#E8E2D8] h-2.5 rounded-full overflow-hidden border border-[#D8D3CB]">
        <div className="bg-[#6B0F1A] h-full transition-all duration-500 rounded-full" style={{ width: `${((activeStep + 1) / steps.length) * 100}%` }} />
      </div>
      <div className="space-y-3 text-left bg-[#F9F7F3] p-4 rounded-2xl border border-[#D8D3CB]">
        {steps.map((st, idx) => {
          ...
          return (
            <div key={idx} className={`flex items-center gap-3 text-xs transition-opacity duration-300 ${isDone ? 'text-emerald-700 font-semibold' : isCurrent ? 'text-[#6B0F1A] font-bold' : 'text-[#5A6270] opacity-50'}`}>
              {isDone ? <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" /> : isCurrent ? <Loader2 className="w-4 h-4 text-[#6B0F1A] animate-spin shrink-0" /> : <div className="w-4 h-4 rounded-full border border-[#D8D3CB] shrink-0" />}
              <span>{st}</span>
            </div>
          );
        })}
